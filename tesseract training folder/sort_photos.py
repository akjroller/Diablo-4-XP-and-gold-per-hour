...
for i, filename in enumerate(processed_files):
    os.rename(os.path.join(processed_dir, filename), os.path.join(processed_dir, f'screenshot_{i}.png'))
...
def handle_click(move):
    if move:
        ...
        shutil.move(os.path.join(unprocessed_dir, files[0]), os.path.join(processed_dir, new_filename))
    else:
